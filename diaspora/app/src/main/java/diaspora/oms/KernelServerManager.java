...
/**
 * Manages Diaspora kernel servers. Tracks which servers are up, which regions each server belongs to, etc.
 * @author iyzhang
 *
 */
public class KernelServerManager {
	Logger logger = Logger.getLogger("diaspora.oms.KernelServerManager");
    ...
    public KernelServer getServer(InetSocketAddress address) {
    	if (servers.containsKey(address)) {
    		...
    		try {
    			...
    			server = (KernelServer) registry.lookup("DiasporaKernelServer");
