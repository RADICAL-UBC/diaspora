...
/** 
 * Diaspora Kernel Server. Runs on every Diaspora node, knows how to talk to the OMS, handles RPCs and has a client for making RPCs.
 * 
 * @author iyzhang
 *
 */
...
public class KernelServerImpl implements KernelServer{
	private static Logger logger = Logger.getLogger("diaspora.kernel.server.KernelServerImpl");
	...
	public KernelServerImpl(InetSocketAddress host, InetSocketAddress omsHost) {
		...
		try {
			...
			oms = (OMSServer) registry.lookup("DiasporaOMS");
		} catch (Exception e) {
	...
	public static void main(String args[]) {
		...
		try {
			...
			registry.rebind("DiasporaKernelServer", stub);
			...
			logger.severe("Cannot start Diaspora Kernel Server");
			e.printStackTrace();
		}
	}
}
