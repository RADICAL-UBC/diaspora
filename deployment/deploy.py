...
def start_oms(oms, server_file, cp_app, cp_diaspora, p_log):
    ...
    cmd += ['-cp ' + cp_app + ':' + cp_diaspora]
    cmd += ['diaspora.oms.OMSServerImpl', hostname, port]
    cmd += [app_class]
...
def start_servers(servers, oms, cp_app, cp_diaspora, p_log):
    for s in servers:
        hostname = s["hostname"]
        port = s["port"]
        ...
        cmd += ['-Djava.util.logging.config.file=\"' + p_log + '\"']
        cmd += ['-cp ' + cp_app + ':' + cp_diaspora]
        cmd += ['diaspora.kernel.server.KernelServerImpl']
...
def start_clients(clients, oms, cp_app, cp_diaspora, p_log):
...
if __name__ == '__main__':
    ...
    # /target/diaspora-1.0-SNAPSHOT.jar is generated after you run 'mvn package' in the 'diaspora' directory
    ...
    p_log = diaspora_home + '/diaspora/logging.properties'

    server_file = "servers.json"
    config = parse_server_config(server_file)
    start_oms(config["oms"], os.path.abspath(server_file), cp_app, cp_diaspora, p_log)
    start_servers(config["servers"], config["oms"], cp_app, cp_diaspora, p_log)
    start_clients(config["clients"], config["oms"], cp_app, cp_diaspora, p_log)
